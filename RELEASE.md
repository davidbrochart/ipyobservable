...
To release a new version of ipyobservable on NPM, first register for an NPM account [here](https://www.npmjs.com/), then log in with `yarn login`. Then:
...
To release a new version of ipyobservable on PyPI, first make sure that the `build` package is installed: `pip install build`.

1. Update `ipyobservable/_version.py`:
   - Update `__version__`
   - Update `NPM_PACKAGE_RANGE` if necessary
2. Commit changes to `_version.py` and tag the release
   ```
   git add ipyobservable/_version.py
